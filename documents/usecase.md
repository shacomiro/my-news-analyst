...
-   **주요 흐름**:
    ...
    4.  **시스템**: 네이버 검색 API를 호출하여 최신 뉴스 기사를 가져옵니다. API 호출 시 **최대 100건의 뉴스 기사를 요청하며, 연관성(sim) 순으로 정렬합니다.** API 호출 성공 시, 가져온 뉴스 기사 목록 중 링크를 기준으로 데이터베이스에 이미 존재하는 기사는 건너뛰고, 새로운 기사만 뉴스 기사 저장소에 저장합니다.

    5.  **시스템**: 새로운 검색 기록을 생성하고, 검색된 뉴스 기사들(신규 및 기존 뉴스 기사 모두 포함)과의 관계를 기록 저장소에 저장합니다. (사용자 로그인 상태에 따라 사용자 정보를 연결하거나 비워둡니다.)
...
-   **예외 흐름**:
    ...
    -   **E1.2 (검색 결과 없음)**: 네이버 검색 API 호출 결과, 해당 키워드와 관련된 뉴스 기사가 전혀 없습니다.
...
-   **UC1: 새로운 뉴스 검색 및 결과 조회**
    ...
    -   **사전 조건**: 사용자가 서비스 웹 페이지에 접근 가능한 상태.
...
-   **API 호출 제한**: 네이버 검색 API의 일일/시간당 호출 횟수 제한을 준수해야 합니다.
