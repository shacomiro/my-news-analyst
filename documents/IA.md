...
1.  **서비스 접속**: 메인 페이지 진입
2.  **키워드 입력 및 검색 실행**: 메인 페이지 검색창에 키워드를 입력하고 검색 버튼을 누릅니다. **(여기서 API 호출 로직이 발생합니다)**: 시스템은 먼저 DB에 **매우 최근에 동일한 키워드로 검색이 완료되어 저장된 최신 결과가 있는지 확인**하고, 있다면 해당 데이터를 활용하여 네이버 검색 API 호출을 방지합니다. 없다면 네이버 검색 API를 호출하여 최신 뉴스 기사를 가져옵니다. 가져온 기사는 DB에 저장됩니다.
...
3.  **키워드 입력 및 검색 실행**: 메인 페이지 또는 로그인 후 검색창에 키워드 입력 및 검색 버튼을 누릅니다. **(여기서 API 호출 로직이 발생합니다)**: DB 내 최신 검색 결과를 우선 활용하고 없을 시 네이버 API를 호출합니다. 가져온 기사는 DB에 저장됩니다.
...
| **메인 검색창**     | 상단 중앙에 위치하며, 어느 페이지에서든 키워드 검색을 시작할 수 있습니다. (검색 버튼 또는 Enter) 이는 **새로운 검색 행위를 시작하는 진입점**이며, API 호출 최적화 로직이 여기서 적용됩니다. 결과는 `/search` 경로로 표시됩니다.                                                            |
...
-   **뉴스 선택 관리**:
    -   '전체 선택' 체크박스
    -   선택된 뉴스 수 표시
-   **더 보기 기능**: '더 보기' 버튼 (클릭 시 추가 뉴스 로드 - 메인 페이지에서의 초기 검색 결과와 동일한 로직으로 DB 또는 API를 통한 추가 데이터 로드)
-   **분석 시작 영역**:
...
-   **새로운 검색 (메인 페이지)**: 키워드 입력 후 Enter 또는 검색 버튼 클릭 시 **메인 페이지에서 백엔드로 검색 요청을 보냅니다.** 백엔드는 요청된 키워드에 대해 DB 최신 결과 확인 후 API 호출(필요시) 로직을 수행하고, 결과를 준비한 뒤 `/search` 경로로 사용자를 이동시킵니다.
-   **뉴스 로드**: `/search` 페이지의 '더 보기' 버튼 클릭 시 비동기적으로 추가 뉴스 목록 로드 (DB 또는 API를 통한 새 데이터 로드). (스크롤 기반 로딩도 고려 가능).
